// motion-graphics-page.js — renders the individual motion graphics project page
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const project = MOTION_PROJECTS.find(p => p.id === id);

  if (!project) {
    document.getElementById('projectInfo').innerHTML =
      '<p style="color:#999">Project not found. <a href="motion-graphics.html" style="color:#4F7CFF">← Back</a></p>';
    return;
  }
  ...
  document.title = title + ' — Lazar Gugleta';
...
switch (project.platform) {
  ...
  if (project.related && project.related.length > 0) {
    const relatedProjects = project.related.map(rid => MOTION_PROJECTS.find(p => p.id === rid)).filter(Boolean);
    ...
    alsoGrid.innerHTML = relatedProjects.map((rp, i) => {
      ...
      const href = isExternal ? rp.externalUrl : `motion-graphics-project.html?id=${rp.id}`;
